...
function App() {
  ...
  const user = { name: "Radinka A...", role: "XI-RPL" }; // Contoh data user, ganti di backend nanti (Di line 56 ada bagian username nanti juga di ganti)
  ...
  return (
    <div>
      <Sidebar activeKey={activePage === "menu" ? "beranda" : activePage} onNavigate={setActivePage} />

      <div style={{ marginLeft: "76px" }}>
        <Topbar user={user} onSearch={(q) => console.log("cari:", q)} onLogout={() => console.log("logout")} />

        <main style={{ padding: "28px" }}>
          {activePage === "beranda" && (
            <>
              <WelcomeHeader schoolName="SMK Negeri 6 Jakarta" userName="Anung" balance={999999} onTopUp={() => console.log("isi saldo")} />
              <CategoryFilter onChange={(key) => console.log("kategori:", key)} />
              <VendorList vendors={vendors} onFilterClick={() => console.log("buka filter")} onViewMenu={handleViewMenu} />
            </>
          )}
